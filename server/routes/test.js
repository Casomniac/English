...
router.post('/', (req, res)=>{
    req.body.forEach(item=>{
        let sql = `INSERT INTO words (word_id, word, word_translation)
               VALUES (DEFAULT, '${item[0]}', '${item[1]}')`
...
router.get('/result', (req, res)=>{
    User.update({
      english_level: req.query.level
    },{
      where: {
        user_vk_id: req.query.userId
      }
    })
      .then(() => {
        User.findOne({
          ...
          .then( user => {
            res.json({level: user.english_level})
          })
      })
});
